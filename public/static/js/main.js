    ...
    this._button.onClick = (ev) => {
        ...
        if (this.map == null) {
            this.map = new google.maps.Map(document.getElementById('googlemap'), {
                zoom: 16,
                // center: { lat: parseFloat(this.options.lattitude), lng: parseFloat(this.options.longitude) },
                center: { lat: parseFloat("35.9242515563965"), lng: parseFloat("-86.8709335327148") },
                mapTypeId: 'satellite',
                rotateControl: false,
...
GoogleMapsLocator.prototype.drawBoundingBox = function (min, max) {
    ...
    var siteLoc = this.geoExtension.lonLatToLmv({ x: parseFloat("35.9242515563965"), y: parseFloat("-86.8709335327148"), z: parseFloat("0") })
    console.log(siteLoc)
    // prepare a polygon with the bounding box information
    var polygon = [];
    polygon.push({ x: min.x, y: min.y });
    polygon.push({ x: min.x, y: siteLoc.y });
    polygon.push({ x: siteLoc.x, y: siteLoc.y });
    polygon.push({ x: siteLoc.x, y: min.y });
...
GoogleMapsLocator.prototype.drawPolygon = function (polygon) {
    ...
    polygon.forEach(function (point) {
        ...
        //var geoLoc = _this.geoExtension.lmvToLonLat(point); ORIGINAL CODE THHIS CONVERTS THE BOUNDING BOX TO GOOGLE COORDINATES WHICH SEEMS TO THROW THINGS OFF
        coords.push({ lat: parseFloat("35.9242515563965"), lng: parseFloat("-86.8709335327148") });
        //coords.push({ lat: geoLoc.y, lng: geoLoc.x });  //ORIGINAL CODE
    });
    var polyOptions = {
        path: coords,
        strokeColor: '#FF0000',
...
GoogleMapsLocator.prototype.cameraChanged = function (camera) {
    ...
    this.map.setCenter({ lat:  parseFloat("35.9242515563965"), lng: parseFloat("-86.8709335327148") });
    // this.map.setCenter({ lat: geoLoc.y, lng: geoLoc.x });  ORIGINAL CODE
}
